...
const publicSurface = [
  readFileSync(resolve("app/page.tsx"), "utf8"),
  readFileSync(resolve("app/layout.tsx"), "utf8"),
  readFileSync(resolve("components/site-header.tsx"), "utf8"),
].join("\n");
const prohibitedPublicExampleMarkers = [
  ...
  "confidence you can take to the bench",
  "All systems operational",
];

for (const marker of prohibitedPublicExampleMarkers) {
  if (publicSurface.includes(marker)) {
...
for (const requiredStatement of ["MODEL RELEASE STATUS", "Inference is not available.", "No approved immutable model artifact"]) {
  if (!publicSurface.includes(requiredStatement)) {
